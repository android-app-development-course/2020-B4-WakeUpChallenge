...
class AlarmListActivity : AppCompatActivity(),AlarmAdapter.InnerClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listView.setOnItemClickListener { parent, view, position, id ->
            adapter.apply {
                selectedID = if (position == selectedID) -1 else selectedID
        ...
        edit.setOnClickListener{
            if (cacheAlarm != null){
                val intent = Intent(this, AlarmEditActivity::class.java)
                intent.putExtra("ACTION", "EDITDEL")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when(requestCode){
            RESULT_CREAT -> if(resultCode == RESULT_OK){
                ...
                val name = bundle?.getString("name")?:"闹钟"
                val repeat=bundle?.getBoolean("repeat")?:false
                val open=bundle?.getBoolean("open")?:true
                val hour = bundle?.getInt("hour")?:0
                val min = bundle?.getInt("min")?:0
                val music = bundle?.getString("music")?:"Default"
    ...
    private fun editOperation(bundle:Bundle?){
        val name = bundle?.getString("name")?:cacheAlarm!!.name
        val repeat = bundle?.getBoolean("repeat")?:cacheAlarm!!.repeat
        val open = bundle?.getBoolean("open")?:cacheAlarm!!.open
        val hour = bundle?.getInt("hour")?:cacheAlarm!!.hour
        val min = bundle?.getInt("min")?:cacheAlarm!!.min
        val music = bundle?.getString("music")?:cacheAlarm!!.music
